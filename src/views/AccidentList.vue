  ...
  <div>
    ...
    <div class="section-card list-card">
      <div class="filter-row">
        ...
        <div class="filter-item"><span>工伤申请：</span><el-select v-model="filters.status" clearable placeholder="全部" size="small"><el-option v-for="item in workflowStatuses" :key="item" :label="item" :value="item"/></el-select></div>
      ...
      <el-table :data="filteredAccidents" stripe class="accident-table" @row-click="goDetail">
        ...
        <el-table-column prop="recordUser" label="记录人" width="110" />
        <el-table-column prop="recordTime" label="记录时间" width="175" />
        <el-table-column label="工伤申请" width="130" align="center"><template slot-scope="scope"><el-tag size="mini" :type="workflowTagType(scope.row)">{{ getWorkflowStatus(scope.row) }}</el-tag></template></el-table-column>
...
export default { name: 'AccidentList', components: { RecordTabs }, data () { return { accidents, filters: { workshop: '', range: [], status: '' }, filterApplied: false, createVisible: false, workflowStatuses: ['无工伤申请', '待发起审批', '审批中', '已结束'], workshops: ['电池回收车间', '浸出净化车间', '维修动力车间', '成品车间'], createForm: { workshop: '', name: '', level: '一般', happenedAt: '', hasInjury: false, responsible: '', note: '' } }; }, computed: { filteredAccidents () { return this.accidents.filter(item => (!this.filters.workshop || item.workshop === this.filters.workshop) && (!this.filters.status || this.getWorkflowStatus(item) === this.filters.status)); } }, methods: { getWorkflowStatus (row) { if (!row.applicationCount) return '无工伤申请'; if (row.workflowStatus === '审批中') return '审批中'; if (row.workflowStatus === '已结束') return '已结束'; return '待发起审批'; }, workflowTagType (row) { const status = this.getWorkflowStatus(row); if (status === '已结束') return 'success'; if (status === '审批中') return 'warning'; if (status === '待发起审批') return 'primary'; return 'info'; }, formatDateTime (date) { const pad = value => String(value).padStart(2, '0'); return date.getFullYear() + '-' + pad(date.getMonth() + 1) + '-' + pad(date.getDate()) + ' ' + pad(date.getHours()) + ':' + pad(date.getMinutes()) + ':' + pad(date.getSeconds()); }, goDetail (row) { this.$router.push({ name: 'AccidentDetail', params: { id: row.id } }); }, resetFilters () { this.filters = { workshop: '', range: [], status: '' }; this.filterApplied = false; }, openCreate () { this.createForm = { workshop: '', name: '', level: '一般', happenedAt: '', hasInjury: false, responsible: '', note: '' }; this.createVisible = true; }, createAccident () { if (!this.createForm.workshop || !this.createForm.name) return this.$message.warning('请先填写车间和事故名称'); const item = { id: 'ACC-' + Date.now(), workshop: this.createForm.workshop, name: this.createForm.name, level: this.createForm.level, type: '其他伤害', happenedAt: this.createForm.happenedAt || '2026-07-22 10:00', location: '', responsible: this.createForm.responsible || '待确认', recordUser: '安环管理员', recordTime: this.formatDateTime(new Date()), injuredCount: 0, applicationCount: 0, status: '未填写', workflowStatus: '未发起', currentNode: '待发起', ccStatus: '待抄送', hrStatus: '待审批', insuranceHandler: '', incident: '', note: this.createForm.note, injuries: [] }; this.accidents.unshift(item); this.createVisible = false; this.$message.success('事故记录已保存'); this.goDetail(item); } } };
</script>
